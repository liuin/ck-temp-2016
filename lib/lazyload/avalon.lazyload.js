...
define(["avalon"], function() {
    ...
    var _delayload = function(options) {
        for (var i = 0, len = lazyElementArr.length; i < len; i++) {
            var imgItem = options.preLoadType === "text" ? lazyElementArr[i].imgEle : lazyElementArr[i],
                eleTop = avalon(imgItem).offset().top,
            ...
            winTop = avalon(window).scrollTop();
            winHeight = avalon(window).height();
    ...
    lazyload.defaults = {
        ...
        delay: 100, //@config  延迟加载时间（毫秒）。也可以设置元素的data-lazyload-itemdelay，替代默认值
